...
### Optimized for SG4 (download only missing)

**On freshly-provisioned sg4 or partially-synced machines, use `download_datasets_missing_only.sh` to skip what's already present:**

```bash
# Analyze what's missing (no download)
bash ai/scripts/bootstrap/download_datasets_missing_only.sh --analyze

# Download only missing public datasets (~28 GB needed on sg4)
bash ai/scripts/bootstrap/download_datasets_missing_only.sh

# Or by feature
bash ai/scripts/bootstrap/download_datasets_missing_only.sh --only safety-fire_detection

# Dry-run first
bash ai/scripts/bootstrap/download_datasets_missing_only.sh --dry-run
```

**See [`BOOTSTRAP_SG4_INSTRUCTIONS.md`](../../BOOTSTRAP_SG4_INSTRUCTIONS.md) for detailed setup on remote machines.**
