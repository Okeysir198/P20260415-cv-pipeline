...
> ID: g | Owner: TBD | Phase: 1 | Status: training | Config: `features/safety-fall-detection/configs/06_training.yaml`
...
Dataset path: `../../dataset_store/fall_detection/` with `{train,val,test}/{images,labels}/` structure. YOLO label format: `class_id cx cy w h` (normalized 0-1).
...
Training uses the pipeline config at `features/safety-fall-detection/configs/06_training.yaml`.

```bash
# Train
uv run core/p06_training/train.py --config features/safety-fall-detection/configs/06_training.yaml

# Evaluate
uv run core/p08_evaluation/evaluate.py \
  --model runs/fall_detection/best.pt \
...
Each release produces a model card at `docs/model_cards/g_fall_detection.md` and a YAML card at `releases/fall_detection/v<N>/model_card.yaml`.

**Model artifacts:**

| Artifact | Format | Path |
|---|---|---|
| PyTorch model | `.pth` | `runs/fall_detection/best.pt` |
| ONNX model | `.onnx` | `runs/fall_detection/export/g_fall_yoloxm_{imgsz}_v{N}.onnx` |
| Training config | `.yaml` | `features/safety-fall-detection/configs/06_training.yaml` |
| Metrics | `.json` | `runs/fall_detection/metrics.json` |
...
| Training run path | `runs/fall_detection/` |
