...
| `hf_multitask` | `multitask_trainer.py::run_multitask_training` | main `.venv/` | Multi-task detection (N tasks share D-FINE trunk; per-task cls heads). Each batch is single-task (collator enforces); eval runs N sequential `super().evaluate()` calls; checkpoint metric is `eval_mean_mAP_50`. Viz/freeze callbacks not wired (deferred). See `features/unified_multitask_phase1/ARCHITECTURE.md` for design. |
