...
| **p06_training** | `core/p06_training/train.py` | `06_training.yaml` (refs `05_data.yaml` by filename) | `<save_dir>/{best.pt, last.pt, test_results.json, data_preview/, val_predictions/{epochs/, best.png, error_analysis/}, test_predictions/{best.png, error_analysis/}}` — full observability tree on `on_train_end`. Backends: `pytorch`, `hf`, `hf_multitask` (shared-trunk + per-task heads — viz callbacks not wired, see `core/p06_training/multitask_trainer.py`). Selecting `backend: paddle` redirects to `core/p06_paddle/train.py` (separate venv). |
| **p06_paddle** | `core/p06_paddle/train.py` | `06_training_paddle_*.yaml` | `<save_dir>/{best.pdparams, test_results.json, model.onnx (after export)}`. Paddle is its own world — runs in `.venv-paddle/`, drives upstream `ppdet.engine.Trainer`. Convergence with the rest of the pipeline happens at ONNX (`core/p06_paddle/export.py` → main-venv ORT path). |
...
**ONNX Runtime is GPU-only by policy; providers are configurable** — `core/p10_inference/predictor.py::_load_onnx_model` defaults to `providers=["CUDAExecutionProvider"]` and raises if any requested provider isn't in `ort.get_available_providers()`. Caller can override via the `DetectionPredictor(providers=...)` kwarg (e.g. `["TensorrtExecutionProvider", "CUDAExecutionProvider"]`). CPU EP is forbidden — never add it as a fallback. If you hit `CUBLAS_STATUS_ALLOC_FAILED` on a saturated shared GPU, free VRAM (kill another process, restart Python).
...
**Paddle is a separate world, not a backend** — instead of bridging paddle into our torch/HF trainer, we drive paddle's upstream `Trainer` directly inside `.venv-paddle/`. The unified `06_training/train.py` dispatcher prints a redirect when `backend: paddle` is selected; the actual entry is `.venv-paddle/bin/python core/p06_paddle/train.py`. Convergence with the rest of the pipeline happens at the ONNX layer — `core/p06_paddle/export.py` writes `model.onnx`, and from there everything (eval, error analysis, inference, demo) goes through the standard main-venv ORT path that already handles YOLOX/HF ONNX. v1 supports detection (PicoDet, PP-YOLOE); cls/seg/kpt drivers can land later inside the same package without touching `core/p06_training` or the unified registry.
