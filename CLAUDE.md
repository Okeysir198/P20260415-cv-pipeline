...
- **`error_analysis_conf_threshold` defaults are per-arch** — `run_post_train_artifacts` (`core/p06_training/post_train.py`) auto-picks **0.25 for YOLOX** vs **0.05 for DETR-family / HF detection** based on `model.output_format` when no explicit threshold is passed. Without this split, YOLOX runs flooded `error_analysis/summary.md` with ~137k FPs at the DETR-tuned 0.05 threshold even though `test_results.json` reported a healthy mAP50 — observed on `our_yolox/runs/seed42` (test mAP50=0.7388 but baseline mAP in summary.md showed 0.0). Override per-config via `training.post_train.error_conf_threshold`. Details: `core/p06_training/CLAUDE.md`.
...
- **`sv.EdgeAnnotator` is 1-indexed; project skeletons are 0-indexed**: every project skeleton constant (`utils.viz.COCO_SKELETON_EDGES`, `core.p06_models.pose_base.COCO_SKELETON` / `MEDIAPIPE_SKELETON`, per-feature `_SKELETON_17`) uses 0-indexed COCO joint IDs (`0=nose, 5=L_shoulder, …`). Supervision's `EdgeAnnotator.annotate` resolves edges as `xy[class - 1]` — passing 0-indexed pairs draws every edge offset by one joint (left arm rendered as eye→elbow, etc.). `utils.viz.annotate_keypoints` adds 1 internally, so any caller routing through it is fine; **direct** callers of `sv.EdgeAnnotator(edges=…)` must convert with `[(a + 1, b + 1) for a, b in skeleton]`. Also: hidden-keypoint filtering — supervision's missing-edge check is `np.allclose(xy, 0)`, not confidence-based; `annotate_keypoints` zeroes out hidden xy on the EdgeAnnotator input so those edges are skipped, and moves them to `-1e6` on the VertexAnnotator input so circles clip off-canvas.
- **`classification_banner` is channel-opaque**: the `bg_color_rgb` / `text_color_rgb` args are written **raw** via `cv2.putText` onto a freshly allocated banner strip, so the helper does *not* convert RGB→BGR. Callers must pass the tuple in the **scene's** actual channel order — BGR for BGR scenes (`(80, 80, 255)` for red), RGB for RGB scenes (`(255, 80, 80)` for red). The `_rgb` suffix on the arg name is a misnomer kept for back-compat. Symmetric tuples (`(30, 30, 30)`, `(255, 255, 255)`) are channel-invariant and don't trip this.
...
- **ImageNet normalization is not always optimal** — verified on `safety-fire_detection` (2026-05-01): fire imagery is ~10% darker / 20% wider variance than ImageNet stats, so applying `mean=[0.485, 0.456, 0.406] / std=[0.229, 0.224, 0.225]` over-normalizes and degrades the R50 backbone features. Setting `tensor_prep.normalize: false` (rescale only) lifted test mAP@0.5 from 0.320 → 0.409 (+0.089) and train mAP from 0.65 → 0.72. For any new detection feature, test no-normalize as a baseline before assuming ImageNet stats are correct. Compute dataset mean/std on a 200-image sample to decide between (a) skip normalize, (b) use dataset-specific normalize, or (c) keep ImageNet.
- **Roboflow split-after-augmentation leakage** — Roboflow exports name files `<source_id>_jpg.rf.<random_hash>.jpg` where the same source image appears multiple times with different augmentations applied. If splitting happens AFTER augmentation, sibling augmentations land in different splits → effective leakage. `safety-fire_detection` had ~50% of val/test leaked from train this way (74,707 cross-split pairs at pHash hamming ≤ 6). The canonical fix lives in `core/p00_data_prep/` — set `dedup.enabled: true` (with `split_strategy: per_source_with_temporal` for multi-source / video-heavy data) in `00_data_preparation.yaml`. `scripts/dedup_split.py` is now a thin wrapper around the same module, kept for re-deduping an existing `training_ready/<name>/` without re-running p00. Likely affects all Roboflow-sourced datasets (helmet, shoes — both Roboflow); audit before training.
- **DETR confidence threshold matters a lot** — DETR-family sigmoid scores cap around 0.2 (small dynamic range). Default `score_threshold=0.05` floods the FP count and can hurt smoke/grey-region classes by 5×+. Use `scripts/threshold_sweep.py --run <ckpt-dir>` to find the F1-optimal operating point per class before setting `model.conf` in `10_inference.yaml`. Verified on fire RT-DETR: 0.075 lifted smoke F1 0.36 → 0.43 (+0.07), fire F1 0.68 → 0.73 (+0.05) — pure inference-time fix, no retrain. Keep `evaluation.score_threshold: 0.0` in TRAINING configs (canonical mAP uses full PR curve); the operating point belongs in `10_inference.yaml`.
