...
- **Quick val overstates mAP vs full val — use `val_full_interval: 0` for production runs that select a released checkpoint**: best.pth is saved on whichever val runs that epoch. At `val_full_interval: 5` (the default), epochs 1-4/6-9/… only see the 20% quick-val subset, which ran ~0.05 mAP higher than full val on fire_detection (20% subset → easier/noisier). The ep51 quick-val best=0.51 was selected at a moment the full-val mAP was 0.45. For arch comparison / HPO where relative ordering matters and wall-time is tight, fast val is fine; for the *final* training run that produces the release checkpoint, set `val_full_interval: 0` so checkpoint selection uses ground-truth numbers. Accept ~2× per-epoch wall time in exchange.
...
- **TTA wins big on smoke**: `scripts/yolox_tta_eval.py` runs multi-scale (default 512/640/768) × optional h-flip inference over val and reports mAP@0.5. On the fire_detection YOLOX-M official ep51 checkpoint: baseline 0.442 → 0.492 mAP (+11%), with smoke AP climbing +24% (0.354 → 0.438) — scale-variant smoke gains most. Deployment cost: 6 inference passes per image (~6× slower). Invariants: use the SAME `build_transforms(is_train=False)` as the evaluator (so inputs are ImageNet-normalized); downscale the normalized tensor with `F.interpolate` — no need to renormalize after resize. Unflip boxes with `x ← W - x`, then scale back to `ref_size` before merging, then class-wise NMS.
- **RT-DETRv2 lr=0.00016 diverges on full fire_detection** (arch-comparison winner on 10% data). Post-warmup (after ep15) the full-val mAP peaks ~ep10 at 0.30, then collapses to 0.06 by ep35. The HPO was run on 5% data + 5 epochs — the sweet-spot LR there is too hot for full data + 150 epochs. On full data use `lr=0.0001` (not 0.00016) and `patience=50` (not 30) to survive early oscillation. Same `val_full_interval=0` applies. Even at lr=0.0001, RT-DETRv2 plateaus at ~0.335 mAP by ep17 and doesn't climb further on full data — per HF/Roboflow canonical fine-tune recipes, lr=5e-5 + 20-30 epochs + warmup=2 is the better recipe for small-class fine-tune.
- **YOLOX postprocess had a double-sigmoid bug** (fixed): both the custom `_DecoupledHead` (`yolox.py:555-559`) and the official adapter (`decode_in_inference=True`) sigmoid obj+cls inside the model's eval-mode forward. `_postprocess_yolox` (`p06_training/postprocess.py`) was then applying sigmoid AGAIN — every score squashed to [0.25, 0.55], making `conf_threshold` effectively meaningless. mAP was correct because sigmoid is monotonic (preserves PR ordering), but: (1) `val_predictions/epoch_NNN.png` showed "green walls" (thousands of boxes per image), (2) production deployment confidence was broken (0.5 meant "random noise", not "50% sure"), (3) `conf_threshold=0.42` at inference let almost everything through. After fix, scores are real [0,1] probabilities; mAP unchanged within rounding. Any prior val_predictions viz is cosmetically broken but stored metrics are valid.
