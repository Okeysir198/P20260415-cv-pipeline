...
- **HF Trainer ghost-folder footgun: `logging.run_name` becomes the feature folder**. `hf_trainer.py:736-740` derives `output_dir` via `generate_run_dir(run_name or project or config_path.parent.parent.name, ...)` — `run_name` wins if set. A stale `logging.run_name: <anything>` in a `06_training_*.yaml` (e.g. leftover `fire_rtdetr_r18_bundleB`) creates `features/<run_name>/runs/...` instead of landing under the actual feature folder. The pytorch backend uses `feature_name_from_config_path()` directly and is immune. Fix: drop `run_name` from HF-backend training configs, or set it to exactly the feature folder name. All 10 DETR configs (5 RT-DETR + 5 D-FINE) had this dropped on 2026-04-21.
