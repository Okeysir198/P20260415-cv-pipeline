"""Reproducible event-level robustness eval for safety-poketenashi_point_and_call.

Runs the orchestrator on every video listed in ``eval/ground_truth.json``,
classifies each emitted ``point_and_call_done`` event against the labelled
gesture windows, and writes a timestamped report. Auto-updates the feature's
CLAUDE.md "Status & investigation log → A. Current evaluation status" table
between the ``<!-- AUTO:section_a -->`` markers so anyone reading the doc sees
...
def _classify_events(events_t: list[float], windows: list[list[float]]) -> dict:
    """Classify each event timestamp against the labelled windows.
...
def _process_video(
    ...
    events: list[float] = []
    counts = {"point_left": 0, "point_right": 0, "point_front": 0,
              "invalid": 0, "neutral": 0, "no_person": 0}
    fi = 0
    ...
    while True:
        ...
        lbl = out.current_label or "no_person"
        counts[lbl] = counts.get(lbl, 0) + 1
        if "point_and_call_done" in out.alerts:
...
def _update_claude_md(section_a: str) -> bool:
    ...
    if _SECTION_A_BEGIN in text and _SECTION_A_END in text:
        ...
        # Markers absent: append a fresh status section below the title.
        marker_block = (
            f"\n{_SECTION_A_BEGIN}\n{section_a}\n{_SECTION_A_END}\n"
        )
        new_text = text + "\n" + marker_block
    ...
    if args.against:
        try:
            ...
            for k in ("tp", "fp", "fn", "precision", "recall", "f1"):
                ...
                print(f"    {k}: {old} -> {cur} ({'+' if (isinstance(d,(int,float)) and d>=0) else ''}{d})")

    if not args.no_update_claude_md:
        section_a = _build_section_a_markdown(per_video, agg, when)
        if _update_claude_md(section_a):
            print(f"  Updated CLAUDE.md section A.")
        else:
            print(f"  CLAUDE.md unchanged (no markers found and content already current).")
