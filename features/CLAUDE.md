...
| ~~`safety-fire_smoke_fall`~~ | ~~Detection (combined 3-class)~~ | — | — | — | ⛔ **Retired 2026-05-19** — superseded by separate `safety-fire_detection` + `safety-fall-detection` per "1 dataset per task" decision. `training_ready/fire_smoke_fall/` deleted; feature folder retained for the 0.584 mAP champion checkpoint. |
