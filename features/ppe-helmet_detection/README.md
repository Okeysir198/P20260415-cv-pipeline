...
Hard-hat PPE compliance — helmet on / off on detected persons.
...
| Dataset | `dataset_store/training_ready/helmet_detection/` |
| Dataset report | `../../dataset_store/training_ready/helmet_detection/DATASET_REPORT.md` |

## Layout

```
features/ppe-helmet_detection/
...
uv run python core/p08_evaluation/evaluate.py \
  --model features/ppe-helmet_detection/runs/best.pt \
  --config features/ppe-helmet_detection/configs/05_data.yaml

# Export to ONNX
uv run python core/p09_export/export.py \
  --model features/ppe-helmet_detection/runs/best.pt \
  --training-config features/ppe-helmet_detection/configs/06_training.yaml

# Inference with alert logic
uv run python core/p10_inference/video.py \
  --config features/ppe-helmet_detection/configs/10_inference.yaml \
  --video features/ppe-helmet_detection/samples/demo.mp4
```
...
training:
  backend: custom
  custom_trainer_class: features.ppe-helmet_detection.code.custom_trainer.MyTrainer
```
