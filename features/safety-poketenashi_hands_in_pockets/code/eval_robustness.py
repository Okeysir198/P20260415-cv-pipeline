"""Reproducible event-level robustness eval for safety-poketenashi_hands_in_pockets.

Runs the predictor on every video listed in ``eval/ground_truth.json``,
classifies each frame whose alerts contain ``hands_in_pockets`` as an event,
collapses contiguous trigger frames into events, then scores them against the
labelled violation windows. Writes a timestamped report and auto-rewrites the
feature CLAUDE.md "Status & investigation log → A. Current evaluation status"
table between the ``<!-- AUTO:section_a -->`` markers.
...
def _frames_to_events(triggered_t: list[float], gap_s: float = 1.0) -> list[tuple[float, float]]:
    """Collapse consecutive triggered-frame timestamps into [start, end] events.

    Two trigger frames more than ``gap_s`` seconds apart become separate events.
    """
    if not triggered_t:
        return []
    events: list[tuple[float, float]] = []
    start = prev = triggered_t[0]
    for t in triggered_t[1:]:
        if t - prev > gap_s:
...
def _classify_events(
    events: list[tuple[float, float]], windows: list[list[float]]
) -> dict:
    """Classify each event interval against the labelled windows.

    An event is TP if its midpoint falls inside any GT window (each window can
    match at most one TP). FP = event with no overlapping window. FN = window
    with no overlapping event.
    """
    matched: set[int] = set()
    fp = 0
    for s, e in events:
...
def _process_video(name: str, video_path: Path, windows: list[list[float]]) -> dict:
    ...
    while True:
        ...
        if _ALERT in out["alerts"]:
            triggered_t.append(fi / fps)
        fi += 1
    cap.release()
    elapsed = time.perf_counter() - t0

    events = _frames_to_events(triggered_t)
    cls = _classify_events(events, windows)
    return {
        "duration_s": round(duration, 1),
        "frames": fi,
        "frames_with_person": n_frames_with_person,
        "elapsed_s": round(elapsed, 1),
        "n_triggered_frames": len(triggered_t),
        "events": [[round(s, 2), round(e, 2)] for s, e in events],
...
def _build_section_a_markdown(per_video: dict[str, dict], agg: dict, when: str) -> str:
    ...
    for name in sorted(per_video.keys()):
        ...
        lines.append(
            f"| `{name}` | {v['duration_s']:.0f} s | {windows} | "
            f"{match_cell} | {_verdict(v)} |"
        )
    return "\n".join(lines)
