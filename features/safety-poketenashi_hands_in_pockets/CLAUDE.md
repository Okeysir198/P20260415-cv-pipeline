...
<!-- last auto-run: 2026-04-30 04:25 UTC -->
...
- **2026-04-30 (structural escalation attempt — reverted)** — Wired ByteTrack into the predictor and added per-track temporal persistence (sliding window: alert fires only when ≥ ratio × K frames triggered for any single track). Tried (window=30, ratio=0.7), (30, 0.3) — both produced **F1 = 0.000**. Massive precision wins (FPs 91 → 1-6 across runs; promotion/railway/autotech all reached TN ✅) but **both TPs died**: per-track triggered density on the 2 positive clips falls below 30 % over any 30-frame window. Diagnosis is unclear without per-frame debug data: either (a) ByteTrack fragments the actor across multiple track IDs (occlusion + person-box jitter), each fragment seeing only sparse triggers, or (b) the elbow gate post-v1.1 already produces sparse positives even on real in-pocket walks because the actor is moving (slight shoulder roll, brief gait moments where elbow-Y rises above the midpoint). Reverted predictor + YAML to v1.1 state. The structural pattern that worked for `point_and_call` (per-track FSM gating with zone polygons) maps less cleanly onto a continuous-posture rule like this one — point_and_call's signal is contextual ("did the gesture happen near the curb"), but hands_in_pockets is a sustained posture that the actor either holds or doesn't, with no zone-based discriminator. **Recommended next path: per-frame debug dump (run `code/dump_debug.py` on the 2 TPs) before any further structural attempt** — set thresholds from data, not from guesses. Or escalate to the ML head route (small MLP on kpt features), since pure rule + temporal smoothing has plateaued at F1 ≈ 0.08.
