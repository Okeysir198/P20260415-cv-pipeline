...
| [safety-fall_classification](./safety-fall_classification/README.md) | `efficientnetv2_rw_s.ra2_in1k.bin` single-frame (Apache-2.0); `slowfast_r50_k400.pyth` temporal (Apache-2.0) | 46 (24 files + 22 symlinks) | 3.5 GB | Clean single-frame + SlowFast / X3D / MoViNet; VideoMAE / V2, UniFormerV2 are CC-BY-NC (benchmark only) | EfficientNetV2-S + VideoMAE-S evaluated on stills; temporal models downloaded but not yet run. |
...
| [safety-poketenashi](./safety-poketenashi/README.md) | `dw-ll_ucoco_384.onnx` WholeBody-133 (Apache-2.0) | 16 | 3.3 GB | Clean for DWPose / RTMW / RTMPose; Sapiens 0.6B non-commercial (benchmark only) | DWPose-L evaluated against body-17 baseline on samples; wholebody runs as expected. |
